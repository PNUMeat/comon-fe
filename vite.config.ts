...
const firebaseMessagingSwPlugin = (mode: string): Plugin => {
  ...
  const renderServiceWorker = () => {
    ...
    return source.replace(
      /__(VITE_FIREBASE_[A-Z0-9_]+?)__/g,
      (_, key: string) => JSON.stringify(env[key] ?? '')
    );
  };
  ...
  return {
    ...
    closeBundle() {
      ...
      fs.writeFileSync(
        path.resolve(outDir, 'firebase-messaging-sw.js'),
        renderServiceWorker()
      );
    },
  };
