...
declare global {
  interface Window {
    ChannelIO?: ChannelIOInterface;
    ChannelIOInitialized?: boolean;
  }
}

interface ChannelIOCommandMap {
  boot: [BootOption, Callback?];
  shutdown: [];
  showMessenger: [];
  hideMessenger: [];
  ...
  onBadgeChanged: [(unread: number, alert: number) => void];
  onChatCreated: [() => void];
  onFollowUpChanged: [(profile: FollowUpProfile) => void];
  onUrlClicked: [(url: string) => void];
  clearCallbacks: [];
  updateUser: [UpdateUserInfo, Callback?];
...
type ChannelIOCommandName = keyof ChannelIOCommandMap;

interface ChannelIOInterface {
  <T extends ChannelIOCommandName>(
    method: T,
    ...args: ChannelIOCommandMap[T]
  ): void;
  c?: (...args: unknown[]) => void;
  q?: unknown[][];
}
...
export class ChannelService {
  loadScript() {
    (function () {
      ...
      const ch = function (method: ChannelIOCommandName, ...args: unknown[]) {
        ch.c?.(method, ...args);
      } as ChannelIOInterface;
      ch.q = [];
